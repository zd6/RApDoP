function res = non_convex_npolygon_computation
...
for i = 6:2:60
    pts = zeros(2, i);
    for j = 1:i
        pts(1,j) = (1-0.51*(mod(j, 2)))*cosd(360/i*j);
        pts(2,j) = (1-0.51*(mod(j, 2)))*sind(360/i*j);
    end
    config.pts = pts;
    starttime = tic();
    ...
    stat_table(i/2,:) = {i, untuned_stat.max, untuned_stat.mean, untuned_stat.median, untuned_stat.std, untuned_stat.range, ...
    ...
    save('.\test\Stat_result_noncov_new','stat_table')
    save('.\test\raw_result_noncov_new','raw_data_tuned', 'raw_data_untuned')
end
end